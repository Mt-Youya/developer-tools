...
export async function onRequestGet(context) {
  try {
    ...
    const _count = url.searchParams.get("count") || "50"
...
async function fetchSteamFreeGames() {
  try {
    ...
    if (data.specials?.items) {
      games = games.concat(data.specials.items.filter((item) => item.final_price === 0 || item.original_price === 0))
    }

    if (data.coming_soon?.items) {
      games = games.concat(data.coming_soon.items.filter((item) => item.final_price === 0 || item.original_price === 0))
    }
...
function parseSteamSearchResults(html) {
  try {
    ...
    const gameRegex =
      /<a href="https:\/\/store\.steampowered\.com\/app\/(\d+)\/[^"]*"[^>]*>.*?<span class="title">([^<]+)<\/span>/gs
    let match
