...
function Folder({ color = "#5227FF", size = 1, items = [], className = "" }: FolderProps) {
  "use memo"
  const maxItems = 3
  ...
  function handlePaperMouseMove(e: MouseEvent<HTMLDivElement, globalThis.MouseEvent>, index: number) {
    if (!open) return
  ...
  function handlePaperMouseLeave(_e: MouseEvent<HTMLDivElement, globalThis.MouseEvent>, index: number) {
    setPaperOffsets((prev) => {
      const newOffsets = [...prev]
