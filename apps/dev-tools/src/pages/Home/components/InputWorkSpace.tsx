...
function assignKeysToValues(target: string[], source: Dictionary) {
  ...
  return target.reduce(
    (prev, current) => (keys.includes(current) ? { ...prev, [current]: source[current] } : prev),
    {} as Dictionary
  )
}
...
function InputWorkSpace() {
  ...
  function handleCopy() {
    const val = inputText.replaceAll(/\d+/g, "").replaceAll(/^ {2}$/g, "")
    copyToClipboard(val)
  }
  ...
  return (
    <Card className="border-0 shadow-lg bg-white/90 backdrop-blur">
      ...
      <CardContent className="space-y-4">
        ...
        <Button variant="outline" className="w-full" onClick={() => setInputText(sampleData)}>
          使用样本数据
        </Button>
