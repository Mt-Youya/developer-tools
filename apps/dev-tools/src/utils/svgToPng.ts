import type { Fn, PromiseReturn } from "@/types/utils"
...
export async function svgToPng(xml: string) {
  const base64 = window.btoa(
    encodeURIComponent(xml).replace(/%([0-9A-F]{2})/g, (_, p1) => String.fromCharCode(parseInt(p1, 16)))
  )
