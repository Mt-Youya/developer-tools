  ...
  "focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 rounded-md border border-transparent bg-clip-padding text-sm font-medium focus-visible:ring-[3px] aria-invalid:ring-[3px] [&_svg:not([class*='size-'])]:size-4 inline-flex items-center justify-center whitespace-nowrap transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none group/button select-none",
  {
    variants: {
      ...
      size: {
        ...
        md: "h-10 gap-1.5 px-2.5 in-data-[slot=button-group]:rounded-md has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
        lg: "h-11 gap-1.5 px-3 in-data-[slot=button-group]:rounded-md has-data-[icon=inline-end]:pr-2.5 has-data-[icon=inline-start]:pl-2.5",
        icon: "size-9",
        "icon-xs":
...
export type ButtonProps = ButtonPrimitive.Props & { loading?: boolean; iconPosition?: "left" | "right" } & VariantProps<
...
function Button({ className, variant, size, loading, ...props }: ButtonProps) {
  ...
  if (loading) {
    const { iconPosition = "left", children } = props
    return (
      <ButtonPrimitive
        disabled
        data-slot="button"
        ...
        {...props}
      >
        {iconPosition === "left" && <Spinner size="xs" data-icon="inline-start" />}
        {children}
        {iconPosition === "right" && <Spinner size="xs" data-icon="inline-end" />}
      </ButtonPrimitive>
    )
  }
