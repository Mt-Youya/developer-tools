...
import type { AdvanceProps } from "./props"

export function CardSpotlight({ title, content }: AdvanceProps) {
  "use memo"
  useEffect(() => {
    ...
    <div className="h-max w-max">
      <div className="spotlight-card group bg-border relative overflow-hidden rounded-xl p-px transition-all duration-300 ease-in-out">
        <Card className="group-hover:bg-card/90 max-w-80 border-none transition-all duration-300 ease-in-out group-hover:backdrop-blur-[20px]">
          <CardHeader>
            <CardTitle>{title || "Hover for the Glow-Up"}</CardTitle>
          </CardHeader>
          <CardContent>
            {content || "Glide your cursor here and watch magic unfold — an experience designed just for you."}
          </CardContent>
        </Card>
